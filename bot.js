...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('794655386011566120');
  }, ms(sürejail));
}
})
...
client.on('guildMemberAdd', async(member) => {
let mute = member.guild.roles.cache.find(r => r.id === "794655385214255165");
let mutelimi = db.fetch(`muteli_${member.guild.id + member.id}`)
let süre = db.fetch(`süre_${member.id + member.guild.id}`)
if (!mutelimi) return;
if (mutelimi == "muteli") {
member.roles.add("794655385214255165")
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('794655385214255165');
  }, ms(süre));
}
})
        ...
        if (reklam.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("ADMINISTRATOR")) {
                  msg.delete();
                    return msg.channel.send(new Discord.MessageEmbed().setDescription(`${msg.author} Bu Sunucuda Reklam Yapamazsın!`).setColor('0x800d0d').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
...
//ses//
client.on('ready', ()=>{
 client.channels.cache.get("794655340750307370").join()
    }) 
//ses//
